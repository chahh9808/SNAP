...
#Main data loader for ONTTA
def load_cifar10c_bybatch(
        n_examples: int,
        idx: int = 0,
        datahelper: DataHelper = None,
        _: Callable = PREPROCESSINGS[None]
) -> Tuple[torch.Tensor, torch.Tensor]:
    return load_corruptions_cifar_bybatch(n_examples,idx,datahelper)
...
def load_corruptions_cifar_bybatch(
        n_examples: int,
        idx: int = 0,
        datahelper: DataHelper = None) -> Tuple[torch.Tensor, torch.Tensor]:
    assert datahelper is not None
    x_test, y_test = datahelper.get_np()
    x_test, y_test = x_test[idx*n_examples:(idx+1)*n_examples], y_test[idx*n_examples:(idx+1)*n_examples]
