...
class SelectedRotateImageFolder(datasets.ImageFolder):
    def __init__(self, root, train_transform, original=True, rotation=True,
    ...
    #     self.samples = [(path, idx) for (path, idx) in self.original_samples if
    #                     idx in self.target_class_index]
...
def prepare_train_dataset(args):
    ...
    trset = SelectedRotateImageFolder(traindir, tr_transforms, original=True,
...
def prepare_train_dataloader(args, trset=None, sampler=None):
    if sampler is None:
        ...
        trloader = torch.utils.data.DataLoader(
            ...
            drop_last=True)  # sampler=None shuffle=True,
    return trloader, train_sampler


def prepare_imagenet_test_data(corruption, level, common_corruptions, batch_size, use_transforms=True,
                               workers=1):
    """TODO Refactor.
    The function do:
    * Set transforms according to corruptions.
    * With SelectedRotateImageFolder, we can get subset. This is not necessarily done in this way.
    """
    ...
    if corruption == 'original':
        ...
        validdir = os.path.join(DATA_PATHS['IN'], 'val')
        ...
        validdir = os.path.join(DATA_PATHS['IN-C'], corruption, str(level))
        teset = SelectedRotateImageFolder(validdir, te_transforms_local, original=False,
                                          rotation=False,
                                          rotation_transform=rotation_te_transforms)
    else:
    ...
    teloader = torch.utils.data.DataLoader(teset, batch_size=batch_size,
                                           shuffle=True,
